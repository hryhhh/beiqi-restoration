...
页面采用上下流程式布局，拆分为 5 个区块，强调“先生成预览，再决定是否保存回壁画”，避免页面退化成单纯的后台表单。
...
- 已有标注路径只读取坐标和基础摘要，不在工作台中编辑标注正文、病害类型或版本信息
- 手动选区继续使用 `AnnotationCoordinates` 结构
- 手动选区只做临时状态，不创建正式标注记录
- 为控制复杂度，手动选区只提供单次矩形或多边形绘制
- “已有标注”和“手动选区”两条路径互斥；用户切换路径时清空另一侧选择，保持当前提交语义单一

提交规则：

- `整图修复` 只要求已选择壁画且已上传原图，不要求选区
- `局部精修` 必须同时满足：
  - 已选择壁画且已上传原图
  - 至少选择一条已有标注，或已完成一组手动选区
...
- 点击变体只替换当前主结果，不重置参数
- Phase 1 结果区只承载“当前主结果 + 变体候选”，不引入历史版本时间轴或结果审批流
...
- 参数校验、提交前置条件、状态切换和保存文件名生成具备纯函数级测试
